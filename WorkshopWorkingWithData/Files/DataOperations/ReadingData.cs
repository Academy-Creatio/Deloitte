...
namespace WorkshopWorkingWithData.Files.DataOperations
{
    internal sealed class ReadingData
    {
        ...
        internal Tuple<DataTable, string> GetAllDataReverseJoinEsq()
        {
            ...
            // = Inner Join
            // > LEFT OUTER JOIN (default behaviour)
            // < RIGHT OUTER JOIN
            // <> FULL OUTER JOIN

            var sysAdminUnitName = esqResult.AddColumn("[SysAdminUnit:Contact:Id].Name");  // SysAdminUnit.Contact == Contact.Id
        ...
        internal Tuple<DataTable, string> GetContactsWithMinutesEsq(Guid ContactId)
        {
            ...
            var durationInMinutes = 
                esqResult.CreateAggregationFunction(AggregationTypeStrict.Sum, "DurationInMinutes");
