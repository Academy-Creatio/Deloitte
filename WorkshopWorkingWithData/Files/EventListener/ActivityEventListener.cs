...
namespace WorkshopWorkingWithData.Files.EventListener
{
    ...
    class ActivityEventListener : BaseEntityEventListener
    {
        ...
        private static int CountOverlappingActivitiesSelect(Entity activity)
        {
            ...
            DateTime start = TimeZoneInfo.ConvertTimeToUtc(
                activity.GetTypedColumnValue<DateTime>("StartDate"), userTimeZoneInfo);
            DateTime due = TimeZoneInfo.ConvertTimeToUtc(
                activity.GetTypedColumnValue<DateTime>("DueDate"), userTimeZoneInfo);
